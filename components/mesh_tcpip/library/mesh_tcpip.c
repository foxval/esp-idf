...
bool esp_hostname_lookup(const char* hostname, uint32_t* address)
{
    ...
    if (*address == 0) {
        ...
        if (hp) {
            ...
            *address = inet_addr(ipaddr_str);
            printf("%s,%d hostname:%s, ip:%s\n", __func__, __LINE__, hostname, ipaddr_str);
            return true;
        }
    }
    return false;
...
mesh_connect_server()
{
    ...
    MESH_ENTER_CRITICAL();
    if (g_server_sock < 0) g_server_sock = socket(PF_INET, SOCK_STREAM, 0);
    MESH_EXIT_CRITICAL();
    ...
    MESH_LOGE("mesh_tcpip_task_init fail\n");

    CONNECT_SERVER_ERR: MESH_ENTER_CRITICAL();
    if (g_server_sock >= 0)
    close(g_server_sock);
    g_server_sock = MESH_TCPIP_SOCK_INVALID;
    MESH_EXIT_CRITICAL();
            ...
            if (g_server_sock == MESH_TCPIP_SOCK_INVALID
                ...
                xQueueReceive(g_mesh_tcpip_queue_handler, &ctx,
                    ...
                    if (ctx && ctx->buf) {
                        ...
                        MESH_LOGW("Send to server len:%d, DST:"MACSTR"\n",
                                head->len, MAC2STR(head->dst_addr));
            ...
            if (g_server_sock == MESH_TCPIP_SOCK_INVALID
                ...
                if (ctx) {
                    ...
                    if (ctx->buf) {
                        ...
                        int size = recv(g_server_sock, ctx->buf,
                                ESP_MESH_PKT_LEN_MAX, MSG_DONTWAIT);
                        MESH_LOGW("Receive from server, size:%d", size);
                        if (size) {
                            int i;
                            for (i = 0; i < size; i++)
                                printf("%x ", buf[i]);
