        ...
        psram_cmd_mode_t cmd_mode)
{
    ...
    while ( READ_PERI_REG(SPI_MEM_FSM_REG(0)) != 0);
    SET_PERI_REG_MASK(APB_CTRL_HOST_INF_SEL_REG, 1 << 14);

    // Start send data
    SET_PERI_REG_MASK(SPI_MEM_CMD_REG(spi_num), SPI_MEM_USR);
    while ((READ_PERI_REG(SPI_MEM_CMD_REG(spi_num)) & SPI_MEM_USR));
    CLEAR_PERI_REG_MASK(APB_CTRL_HOST_INF_SEL_REG, 1 << 14);
...
void IRAM_ATTR psram_spi_init(psram_spi_num_t spi_num, psram_cache_mode_t mode)
{
    ...
    CLEAR_PERI_REG_MASK(SPI_MEM_SLAVE_REG(spi_num), SPI_MEM_TRANS_DONE << 5);
#endif
...
esp_err_t IRAM_ATTR psram_enable(psram_cache_mode_t mode, psram_vaddr_mode_t vaddrmode)   //psram init
{
    ...
    switch (mode) {
        case PSRAM_CACHE_F80M_S80M:
            ...
            WRITE_PERI_REG(SPI_MEM_ADDR_REG(PSRAM_SPI_3), 32 << 24);
            WRITE_PERI_REG(SPI_MEM_CLOCK_REG(PSRAM_SPI_3), SPI_MEM_CLK_EQU_SYSCLK_M);   //SET 80M AND CLEAR OTHERS
            SET_PERI_REG_MASK(SPI_MEM_CMD_REG(PSRAM_SPI_3), SPI_MEM_FLASH_READ_M);
            uint32_t spi_status;
            while (1) {
                spi_status = READ_PERI_REG(SPI_MEM_FSM_REG(PSRAM_SPI_3));
...
{
#ifdef CONFIG_CHIP_IS_ESP32
    CLEAR_PERI_REG_MASK(SPI_MEM_CLOCK_REG(0), SPI_MEM_CLK_EQU_SYSCLK_M);
    SET_PERI_REG_BITS(SPI_MEM_CLOCK_REG(0), SPI_MEM_CLKDIV_PRE_V, 0, SPI_MEM_CLKDIV_PRE_S);
    SET_PERI_REG_BITS(SPI_MEM_CLOCK_REG(0), SPI_MEM_CLKCNT_N, 1, SPI_MEM_CLKCNT_N_S);
    ...
    switch (psram_cache_mode) {
        case PSRAM_CACHE_F80M_S80M:
            ...
            SET_PERI_REG_BITS(SPI_MEM_CACHE_SCTRL_REG(0), SPI_MEM_SRAM_RDUMMY_CYCLELEN_V, PSRAM_FAST_READ_DUMMY + extra_dummy,
    ...
    switch (psram_cache_mode) {
        case PSRAM_CACHE_F80M_S80M: //in this mode , no delay is needed
            ...
            SET_PERI_REG_BITS(SPI_MEM_SRAM_DRD_CMD_REG(0), SPI_MEM_CACHE_SRAM_USR_RD_CMD_VALUE_V, PSRAM_FAST_READ,
        ...
        default:
            ...
            SET_PERI_REG_BITS(SPI_MEM_SRAM_DRD_CMD_REG(0), SPI_MEM_CACHE_SRAM_USR_RD_CMD_VALUE_V, ((PSRAM_FAST_READ) << 8),
    ...
    DPORT_SET_PERI_REG_MASK(DPORT_PRO_CACHE_CTRL_REG, DPORT_PRO_CACHE_ENABLE);
#if !CONFIG_FREERTOS_UNICORE
...
#endif
#else
#if CONFIG_SPIRAM_SIZE <= (DPORT_CACHE_ADDRESS_HIGH - DPORT_CACHE_ADDRESS_LOW)
    REG_SET_BIT(DPORT_CACHE_SOURCE_1_REG, DPORT_PRO_CACHE_D_SOURCE_PRO_DPORT);
    REG_CLR_BIT(DPORT_PRO_DCACHE_CTRL1_REG, DPORT_PRO_DCACHE_MASK_DPORT);
#elif CONFIG_SPIRAM_SIZE <= (DRAM1_ADDRESS_HIGH - DPORT_CACHE_ADDRESS_LOW)
    REG_SET_BIT(DPORT_CACHE_SOURCE_1_REG, DPORT_PRO_CACHE_D_SOURCE_PRO_DPORT);
    REG_CLR_BIT(DPORT_PRO_DCACHE_CTRL1_REG, DPORT_PRO_DCACHE_MASK_DRAM1 | DPORT_PRO_DCACHE_MASK_DPORT);
#else
