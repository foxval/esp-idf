...
tBTM_STATUS BTM_BleReadPhy(UINT16 conn_id, UINT8 *tx_phy, UINT8 *rx_phy)
{
    ...
    btsnd_hcic_ble_read_phy(conn_id);
    // TODO:
    //cb_params.read_phy.rx_phy = ble_rsp_buf[2];
    //cb_params.read_phy.tx_phy = ble_rsp_buf[3];
...
tBTM_STATUS BTM_BleSetPreferPhy(UINT16 conn_handle, UINT8 tx_phy_mask,
                                           UINT8 rx_phy_mask, UINT16 phy_options)
{
    ...
    if ((err = btsnd_hcic_ble_set_phy(conn_handle, all_phys, tx_phy_mask, rx_phy_mask, phy_options)) != HCI_SUCCESS) {
        BTM_TRACE_ERROR("%s, fail to send the hci command, the error code = %s(0x%x)", 
                        __func__, btm_ble_hci_status_to_str(err), err);
        status = BTM_ILLEGAL_VALUE;
    }
...
tBTM_STATUS BTM_BlePeriodicAdvCfgDataRaw(UINT8 instance, UINT16 len, UINT8 *data)
{
    ...
    if (len > controller_get_interface()->ble_get_adv_max_len()) {
        BTM_TRACE_ERROR("%s, The adv data len(%d) is longer then the controller adv max len(%d)", 
            __func__, len, controller_get_interface()->ble_get_adv_max_len());
        status = BTM_ILLEGAL_VALUE;
    }
...
tBTM_STATUS BTM_BleSetExtendedScanParams(tBTM_BLE_EXT_SCAN_PARAMS *params, UINT8 limit)
{
    UINT8 phy_mask = 0;
    tHCI_STATUS err = HCI_SUCCESS;
    tBTM_STATUS status = BTM_SUCCESS;
...
void btm_ble_update_phy_evt(tBTM_BLE_UPDATE_PHY *params)
{
    ...
    memcpy(&cb_params.phy_update.phy_update, params, sizeof(tBTM_BLE_UPDATE_PHY));
