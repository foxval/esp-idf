...
#define HCI_GET_CMD_BUF(paramlen)       ((BT_HDR *)osi_calloc(HCI_CMD_BUF_SIZE))
#if (BLE_50_FEATURE_SUPPORT == TRUE)
static uint8_t status;
static osi_mutex_t sync_mutex;



void btsnd_hcic_ble_sync_mutex_init(void)
{
    osi_mutex_new(&sync_mutex);
}

void btsnd_hcic_ble_sync_mutex_deinit(void)
{
    osi_mutex_free(&sync_mutex);
}

osi_mutex_t *btsnd_hcic_ble_get_sync_mutex(void)
{
    return &sync_mutex;
}
...
UINT8 btsnd_hcic_ble_read_phy(UINT16 conn_handle)
{
    BT_HDR *p;
    UINT8 *pp;
