...
static size_t _udp_pcb_write(tcpip_adapter_if_t tcpip_if, mdns_pcb_type_t pcb_type, const ip_addr_t *ip, uint16_t port, uint8_t * data, size_t len)
{
    ...
    memcpy(dst, data, len);
    err_t err = udp_sendto(_mdns_server->interfaces[tcpip_if].pcbs[pcb_type].pcb, pbt, ip, port);
    pbuf_free(pbt);
    if (err) {
        return 0;
    }
...
static uint16_t _mdns_append_question_to_tx_packet(uint8_t * packet, uint16_t * index, mdns_out_question_t * q)
{
    const char * str[4];
    uint8_t str_index = 0;
    uint8_t part_length;
...
static bool _mdns_append_answer_to_tx_packet(uint8_t * packet, uint16_t * index, mdns_out_answer_t * answer, tcpip_adapter_if_t tcpip_if){
    if(answer->type == MDNS_TYPE_PTR){
        ...
        return _mdns_append_a_record(packet, index, if_ip_info.ip.addr, answer->flush, answer->bye) > 0;
        ...
        if (!tcpip_adapter_get_ip6_linklocal(tcpip_if, &if_ip6)) {
            ...
            if (i<sizeof(ip6_addr_t)) {
                return _mdns_append_aaaa_record(packet, index, v6addr, answer->flush, answer->bye) > 0;
            }
        }
    }
    return false;
...
static void _mdns_dispatch_tx_packet(mdns_tx_packet_t * p){
    ...
    q = p->questions;
    while(q){
        if(_mdns_append_question_to_tx_packet(packet, &index, q)){
            count++;
        }
        q = q->next;
    }
    ...
    a = p->answers;
    while(a){
        if (_mdns_append_answer_to_tx_packet(packet, &index, a, p->tcpip_if)) {
            count++;
        }
        a = a->next;
    }
    ...
    if(p->dst.type == IPADDR_TYPE_V4){
        _mdns_dbg_printf("To: " IPSTR ", ", IP2STR(&p->dst.u_addr.ip4));
    } else {
        _mdns_dbg_printf("To: " IPV6STR ", ", IPV62STR(p->dst.u_addr.ip6));
    }
...
static void _mdns_create_answer_from_parsed_packet(mdns_parsed_packet_t * parsed_packet)
{
    ...
    if(unicast){
        ...
        if(parsed_packet->pcb_type == PCB_TYPE_V4){
...
static mdns_tx_packet_t * _mdns_create_announce_from_probe(mdns_tx_packet_t * probe){
    ...
    packet->additional = NULL;
    packet->distributed = false;
    packet->tcpip_if = probe->tcpip_if;
    packet->pcb_type = probe->pcb_type;
    packet->flags = MDNS_FLAGS_AUTHORITATIVE;
    packet->port = probe->port;
    memcpy(&packet->dst, &probe->dst, sizeof(ip_addr_t));
...
static bool _mdns_run_scheduler(){
    ...
    if(p->send_at <= t){
        //maybe check if pcb is valid?
        _mdns_server->tx_queue = _mdns_server->tx_queue->next;
        if(_mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].state == PCB_OFF){
            _mdns_free_tx_packet(p);
            return false;
        }
        ...
        switch(_mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].state){
            case PCB_PROBE_1:
                //remove unicast flag
                q = p->questions;
                while(q){
            ...
            case PCB_PROBE_2:
                //re schedule probe and increment state
                _mdns_schedule_tx_packet(p, 250);
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].state = (mdns_pcb_state_t)((uint8_t)(_mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].state) + 1);
                break;
            case PCB_PROBE_3:
                //create announce packet from probe
                ...
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].probe_running = false;
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].probe_ip = false;
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].probe_services_len = 0;
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].failed_probes = 0;
                free(_mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].probe_services);
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].probe_services = NULL;
                //free probe packet
                _mdns_free_tx_packet(p);
                p = a;
            ...
            case PCB_ANNOUNCE_1:
                send_after = 1000;
            case PCB_ANNOUNCE_2:
                //send announce and increment state
                _mdns_schedule_tx_packet(p, send_after);
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].state = (mdns_pcb_state_t)((uint8_t)(_mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].state) + 1);
                break;
            case PCB_ANNOUNCE_3:
                //ToDo: event ready
                _mdns_server->interfaces[p->tcpip_if].pcbs[p->pcb_type].state = PCB_RUNNING;
                //free announce packet
                _mdns_free_tx_packet(p);
                break;
            default:
                _mdns_free_tx_packet(p);
 ...
 */
static void _mdns_add_result(mdns_result_temp_t * r)
{
    ...
    mdns_result_t * o = _mdns_server->search.results;
    _mdns_server->search.results = n;
    n->next = o;
}
...
static int _mdns_check_txt_collision(mdns_service_t * service, const uint8_t * data, size_t len)
{
   size_t data_len = 1;
   uint8_t i;
   if(len == 1 && service->txt_num_items){
       return -1;//we win
   } else if(len > 1 && !service->txt_num_items){
       return 1;//they win
   } else if(len == 1 && !service->txt_num_items){
       return 0;//same
   }
   ...
   for(i=0;i<service->txt_num_items;i++){
       data_len += 1 + strlen(service->txt[i]);
   }
   ...
   for(i=0;i<service->txt_num_items;i++){
       _mdns_append_string(ours, &index, service->txt[i]);
   }
 ...
 */
static mdns_service_t * _mdns_create_service(const char * service, const char * proto, uint16_t port, const char * instance, uint8_t num_items, const char ** txt)
{
    ...
    char ** new_txt = NULL;
    uint8_t new_num_items = 0, i = 0;
    if (num_items) {
        new_txt = (char **)malloc(sizeof(char *) * num_items);
        if (!new_txt) {
            return NULL;
        }
        new_num_items = num_items;
        for(i=0; i<num_items; i++) {
            new_txt[i] = strdup(txt[i]);
            if (!new_txt[i]) {
                new_num_items = i;
                break;
            }
...
static bool _mdns_ip6_to_arpa(tcpip_adapter_if_t tcpip_if, char * ip6_str)
{
    struct ip6_addr if_ip6;
    if (tcpip_adapter_get_ip6_linklocal(tcpip_if, &if_ip6)) {
        return false;
    }
    uint8_t * ip6_buf = (uint8_t *)&if_ip6.addr;
    uint8_t i, s = 0, v;
    for (i=0; i<16; i++) {
        v = ip6_buf[15 - i];
        ip6_str[s++] = _itoc4(v);
        ip6_str[s++] = '.';
        ip6_str[s++] = _itoc4(v >> 4);
        ip6_str[s++] = (i == 15)?0:'.';
    }
    return true;
}

static bool _mdns_ip_to_arpa(tcpip_adapter_if_t tcpip_if, char * ip_str)
{
    tcpip_adapter_ip_info_t if_ip;
    tcpip_adapter_get_ip_info(tcpip_if, &if_ip);
    if (!if_ip.ip.addr) {
        return false;
    }
    uint8_t * ip_buf = (uint8_t *)&if_ip.ip.addr;
    sprintf(ip_str, "%u.%u.%u.%u", ip_buf[3], ip_buf[2], ip_buf[1], ip_buf[0]);
    return true;
}

static bool _mdns_arpa_is_ours(mdns_name_t * name, uint16_t type, tcpip_adapter_if_t tcpip_if)
{
    if(name->domain && name->domain[0] && strcasecmp(name->domain, "arpa") == 0) {
        char ip_str[64] = {0,};
        if (name->proto[0] && strcasecmp(name->proto, "ip6") == 0) {
            if (!_mdns_ip6_to_arpa(tcpip_if, ip_str)) {
                return false;
            }
        } else if(name->proto[0] && strcasecmp(name->proto, "in-addr") == 0){
            if (!_mdns_ip_to_arpa(tcpip_if, ip_str)) {
                return false;
            }
        } else {
            return false;
        }
        return (strcasecmp(name->service, ip_str) == 0);
    }
    return false;
}
*/
...
static bool _mdns_question_matches(mdns_parsed_question_t * question, uint16_t type, mdns_srv_item_t * service){
    ...
    if(type == MDNS_TYPE_A || type == MDNS_TYPE_AAAA){
        ...
        if(!strcasecmp(service->service->service, question->service) && !strcasecmp(service->service->proto, question->proto) && !strcasecmp(MDNS_DEFAULT_DOMAIN, question->domain)){
            return true;
        }
        ...
        if(!strcasecmp(name, question->host) && !strcasecmp(service->service->service, question->service) && !strcasecmp(service->service->proto, question->proto) && !strcasecmp(MDNS_DEFAULT_DOMAIN, question->domain)){
            return true;
        }
    }
...
void mdns_parse_packet(mdns_rx_packet_t * packet)
{
    ...
    if (header.questions) {
        ...
        while(qs--) {
            ...
            if (type == MDNS_TYPE_PTR)          { _mdns_printf("%s.%s%s.%s.%s. PTR ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_SRV)     { _mdns_printf("%s.%s%s.%s.%s. SRV ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_TXT)     { _mdns_printf("%s.%s%s.%s.%s. TXT ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_A)       { _mdns_printf("%s.%s. A ", name->host, name->domain); }
            else if (type == MDNS_TYPE_AAAA)    { _mdns_printf("%s.%s. AAAA ", name->host, name->domain); }
            else if (type == MDNS_TYPE_NSEC)    { _mdns_printf("%s.%s%s.%s.%s. NSEC ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_ANY)     { _mdns_printf("%s.%s%s.%s.%s. ANY ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else                                { _mdns_printf("%s.%s%s.%s.%s. %04X ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain, type); }
    ...
    if(header.questions && !parsed_packet->questions && !parsed_packet->discovery){
        ...
        while(content < (data + len)) {
            ...
            if (type == MDNS_TYPE_PTR)          { _mdns_printf(": %s.%s.%s.%s. PTR ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_SRV)     { _mdns_printf(": %s.%s.%s.%s. SRV ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_TXT)     { _mdns_printf(": %s.%s.%s.%s. TXT ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_A)       { _mdns_printf(": %s.%s. A ", name->host, name->domain); }
            else if (type == MDNS_TYPE_AAAA)    { _mdns_printf(": %s.%s. AAAA ", name->host, name->domain); }
            else if (type == MDNS_TYPE_NSEC)    { _mdns_printf(": %s.%s.%s.%s. NSEC ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_ANY)     { _mdns_printf(": %s.%s.%s.%s. ANY ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_OPT)     { _mdns_printf(": . OPT "); }
            else                                { _mdns_printf(": %s.%s.%s.%s. %04X ", name->host, name->service, name->proto, name->domain, type); }
            ...
            if (type == MDNS_TYPE_PTR) {//instance
                //match service,proto
                ...
                if(search_result){
                    ...
                    if(discovery){
                        ...
                        //it's advertise/answer
                        //but data is the same, so it's not collision
                        //maybe remove scheduled answer if exists?
                        if(!ttl){
                ...
                if(search_result){
                    ...
                    if(col > 0 || _ipv6_address_is_zero(ip6)){
                        do_not_reply = true;
                        if(_mdns_server->interfaces[packet->tcpip_if].pcbs[packet->pcb_type].probe_running){
                            if(col && (parsed_packet->probe || parsed_packet->authoritative)){
                                _mdns_server->interfaces[packet->tcpip_if].pcbs[packet->pcb_type].failed_probes++;
...
void _mdns_enable_pcb(tcpip_adapter_if_t tcpip_if, mdns_pcb_type_t pcb_type)
{
    if(!_mdns_server->interfaces[tcpip_if].pcbs[pcb_type].pcb){
        if(_udp_pcb_init(tcpip_if, pcb_type)){
            return;
        }
    }
...
{
    if(_mdns_server->interfaces[tcpip_if].pcbs[pcb_type].pcb){
        //send bye if proper status?
        _mdns_clear_pcb_tx_queue(tcpip_if, pcb_type);
        _udp_pcb_deinit(tcpip_if, pcb_type);
    }
}
...
static void _mdns_free_action(mdns_action_t * action)
{
    ...
    switch(action->type){
        ...
        case ACTION_SERVICE_TXT_SET:
            for(i=0; i<action->data.srv_txt.len; i++){
                free(action->data.srv_txt.items[i]);
            }
            free(action->data.srv_txt.items);
            break;
        default:
            break;
    }
...
static void _mdns_execute_action(mdns_action_t * action)
{
    ...
    switch(action->type){
        ...
        case ACTION_SERVICE_PORT_SET:
            ...
            break;
        case ACTION_SERVICE_TXT_SET:
            if (action->data.srv_txt.service->service->txt_num_items) {
                uint8_t i;
                for(i=0; i<action->data.srv_txt.service->service->txt_num_items; i++) {
                    free((char *)action->data.srv_txt.service->service->txt[i]);
                }
            }
...
void mdns_handle_system_event(system_event_id_t event)
{
    ...
    ///_mdns_handle_system_event(event);
...
esp_err_t mdns_service_add(const char * service, const char * proto, uint16_t port, const char * instance, uint8_t num_items, const char ** txt)
{
...
esp_err_t mdns_service_txt_set(const char * service, const char * proto, uint8_t num_items, const char ** txt)
{
    uint8_t i;
    ...
    action->data.srv_txt.service = s;
    action->data.srv_txt.len = new_num_items;
    action->data.srv_txt.items = new_txt;
    if(xQueueSend(_mdns_server->action_queue, &action, (portTickType)0) != pdPASS){
...
void mdns_debug_packet(const uint8_t * data, size_t len)
{
    ...
    _mdns_dbg_printf("%s", (header.flags.value == MDNS_FLAGS_AUTHORITATIVE)?"AUTHORITATIVE\n":((header.flags.value == 0)?"\n":" "));
    if(header.flags.value && header.flags.value != MDNS_FLAGS_AUTHORITATIVE){
    ...
    if (header.questions) {
        ...
        while(qs--) {
            ...
            if (type == MDNS_TYPE_PTR)          { _mdns_dbg_printf("%s.%s%s.%s.%s. PTR ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_SRV)     { _mdns_dbg_printf("%s.%s%s.%s.%s. SRV ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_TXT)     { _mdns_dbg_printf("%s.%s%s.%s.%s. TXT ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            ...
            else if (type == MDNS_TYPE_NSEC)    { _mdns_dbg_printf("%s.%s%s.%s.%s. NSEC ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_ANY)     { _mdns_dbg_printf("%s.%s%s.%s.%s. ANY ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain); }
            else                                { _mdns_dbg_printf("%s.%s%s.%s.%s. %04X ", name->host, name->sub?"_sub.":"", name->service, name->proto, name->domain, type); }
    ...
    if (header.answers || header.servers || header.additional) {
        ...
        while(content < (data + len)) {
            ...
            if (type == MDNS_TYPE_PTR)          { _mdns_dbg_printf(": %s%s%s.%s.%s. PTR ", name->host, name->host[0]?".":"", name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_SRV)     { _mdns_dbg_printf(": %s.%s.%s.%s. SRV ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_TXT)     { _mdns_dbg_printf(": %s.%s.%s.%s. TXT ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_A)       { _mdns_dbg_printf(": %s.%s. A ", name->host, name->domain); }
            else if (type == MDNS_TYPE_AAAA)    { _mdns_dbg_printf(": %s.%s. AAAA ", name->host, name->domain); }
            else if (type == MDNS_TYPE_NSEC)    { _mdns_dbg_printf(": %s.%s.%s.%s. NSEC ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_ANY)     { _mdns_dbg_printf(": %s.%s.%s.%s. ANY ", name->host, name->service, name->proto, name->domain); }
            else if (type == MDNS_TYPE_OPT)     { _mdns_dbg_printf(": . OPT "); }
            else                                { _mdns_dbg_printf(": %s.%s.%s.%s. %04X ", name->host, name->service, name->proto, name->domain, type); }
