...
typedef volatile struct {
    ...
    union {
        struct {
            ...
            uint32_t flash_suspending:   1;                 /*The status of flash suspend*/
    ...
    union {
        struct {
            ...
            uint32_t reserved16:    12;                     /*reserved*/
            uint32_t sub_pin:        1;                     /*For SPI0  SRAM and flash work with separate PINs.*/
    ...
    union {
        struct {
            ...
            uint32_t cmd_define:       1;                   /*1: slave mode commands are defined in SPI_SLAVE3. 0: slave mode commands are fixed as 1:  write-status  4: read-status  2: write-buffer and 3: read-buffer.*/
    ...
    union {
        struct {
            ...
            uint32_t usr_sram_oct:         1;               /*For SPI0  In the spi sram mode  spi octal I/O mode enable  1: enable  0:disable*/
    ...
    union {
        struct {
            uint32_t cnt_l:           6;                    /*For SPI0 SRAM interface  it must be equal to spi_clkcnt_N. In the slave mode it must be 0.*/
            uint32_t cnt_h:           6;                    /*For SPI0 SRAM interface  it must be floor((spi_clkcnt_N+1)/2-1). In the slave mode it must be 0.*/
            uint32_t cnt_n:           6;                    /*For SPI0 SRAM interface  it is the divider of spi_clk. So spi_clk frequency is system/(spi_clkdiv_pre+1)/(spi_clkcnt_N+1)*/
            uint32_t div_pre:        13;                    /*For SPI0 SRAM interface  it is pre-divider of spi_clk.*/
            uint32_t equ_sysclk:      1;                    /*For SPI0 SRAM interface  1: spi_clk is eqaul to system 0: spi_clk is divided from system clock.*/
        };
        uint32_t val;
    } sram_clk;
    union {
        struct {
            uint32_t tsus:             16;                  /*PES command is delayed tsus cycle based on apb clock.*/
            uint32_t per_command:       8;                  /*Program/Erase resume command.*/
            uint32_t pes_command:       8;                  /*Program/Erase suspend command.*/
        };
        uint32_t val;
    } flash_sus;
