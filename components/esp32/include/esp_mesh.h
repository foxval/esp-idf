 ...
 *  Examples:
 *  (1)Applications could use WiFi station connect states to decide when to send data to its parent, to root or to outside IP network;
 ...
 *
 *  Over the mesh network, only root is able to access outside IP network.
...
#define MESH_DATA_TODS          (0x08)  /**< identify this packet is target to outside of the mesh network */
 ...
 */
#define MESH_OPT_SEND_GROUP     (7)     /**< data transmission by group; used with esp_mesh_send() */
#define MESH_OPT_RECV_DS_ADDR   (8)     /**< return a remote IP address; used with esp_mesh_recv() */
...
typedef enum {
    ...
    MESH_EVENT_TODS_STATE,              /**< state represents if root is able to access outside IP network */
...
typedef enum {
    ...
    MESH_ROOT,    /**< the only sink of the mesh network. Has the ability to access outside IP network */
...
typedef enum {
    ...
    MESH_TODS_REACHABLE,      /**< root is able to access outside IP network */
...
typedef union {
    ...
    mesh_event_toDS_state_t toDS_state;                    /**< toDS state, nodes should check this state firstly before trying to send packets to
                                                                outside of the mesh network. This state indicates right now if root is capable
 ...
 *            (2)if the packet is outgoing to outside IP network such as an IP server address, translate IPv4:PORT known as "to".
 ...
 *              if the packet is outgoing to root or to outside of the mesh network, MESH_DATA_TODS should be set.
 ...
 *              for upstream data to outside IP network, we try our best to avoid data loss caused by root having been changed, but
 ...
 *              to outside via socket is just from reading esp_mesh_recv_toDS() which avoids unnecessary memory copy.
 *
 * @param     opt  options
 ...
 *            (2)root sends a packet to an internal node, this packet is from outside IP network in case the receiver node responds
 ...
 * @param     flag
 *            MESH_DATA_FROMDS represents data from outside IP network
...
/**
 * @brief     receive a packet targeted to outside IP network
 *            root uses this API to receive packets destined to outside IP network
 ...
 * @param     timeout_ms  wait time if a packet isn't immediately available(0:no wait, portMAX_DELAY:wait forever)
 * @param     flag
 *            MESH_DATA_TODS represents data to outside IP network
 * @param     opt  options desired to receive
 * @param     opt_count  option count desired to receive
 *
 * @return
 ...
 *
 *            In this API, users could specify a desired root address to replace itself or specify a attempts value
 ...
 *
 * @param     reachable  this state represents if root is able to access outside IP network
 *
 * @return
 *    - ESP_OK
 *    - ESP_FAIL
