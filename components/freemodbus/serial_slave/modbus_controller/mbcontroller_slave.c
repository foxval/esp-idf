...
#include "mbcontroller.h"           // for common function pointers
...
static esp_err_t mbcontroller_setup_slave(void* comm_info)
{
    MB_ASSERT(mbs_opts != NULL);
    MB_CHECK((comm_info != NULL), ESP_ERR_INVALID_ARG,
                    "mb wrong communication settings.");

    mb_communication_info_t* comm_settings = (mb_communication_info_t*)comm_info;
    MB_CHECK(((comm_settings->mode == MB_MODE_RTU) || (comm_settings->mode == MB_MODE_ASCII)),
                ESP_ERR_INVALID_ARG, "mb incorrect mode = (0x%x).",
                (uint32_t)comm_settings->mode);
    MB_CHECK((comm_settings->slave_addr <= MB_ADDRESS_MAX),
                ESP_ERR_INVALID_ARG, "mb wrong slave address = (0x%x).",
                (uint32_t)comm_settings->slave_addr);
    MB_CHECK((comm_settings->port <= UART_NUM_2), ESP_ERR_INVALID_ARG,
                "mb wrong port to set = (0x%x).", (uint32_t)comm_settings->port);
    MB_CHECK((comm_settings->parity <= UART_PARITY_EVEN), ESP_ERR_INVALID_ARG,
...
static esp_err_t mbcontroller_start_slave(void)
{
    MB_ASSERT(mbs_opts != NULL);
...
static esp_err_t mbcontroller_destroy_slave(void)
{
    MB_ASSERT(mbs_opts != NULL);
...
static esp_err_t mbcontroller_set_descriptor_slave(const mb_register_area_descriptor_t descr_info)
{
    MB_ASSERT(mbs_opts != NULL);
                                    ...
                                    uint8_t* par_address, uint16_t par_size)
{
    ...
    if (pdTRUE == status) {
        ESP_LOGD(MB_CONTROLLER_TAG, "Queue send parameter info (type, address, size): %d, 0x%.4x, %d",
                par_type, (uint32_t)par_address, par_size);
        error = ESP_OK;
    } else if (errQUEUE_FULL == status) {
        ESP_LOGD(MB_CONTROLLER_TAG, "Parameter queue is overflowed.");
    }
    return error;
}
...
static esp_err_t send_param_access_notification(mb_event_group_t event)
{
    ...
    mb_event_group_t bits = (mb_event_group_t)xEventGroupSetBits(mbs_opts->mbs_event_group,
                                                                    (EventBits_t)event);
    if (bits & event) {
        ESP_LOGD(MB_CONTROLLER_TAG, "The MB_REG_CHANGE_EVENT = 0x%.2x is set.", (uint8_t)event);
        err = ESP_OK;
    }
    return err;
}
...
static mb_event_group_t mbcontroller_check_event_slave(mb_event_group_t group)
{
    MB_ASSERT(mbs_opts != NULL);
    MB_ASSERT(mbs_opts->mbs_event_group != NULL);
...
static esp_err_t mbcontroller_get_param_info_slave(mb_param_info_t* reg_info, uint32_t timeout)
{
    MB_ASSERT(mbs_opts != NULL);
    esp_err_t err = ESP_ERR_TIMEOUT;
    MB_CHECK((mbs_opts->mbs_notification_queue_handle != NULL),
...
esp_err_t mbcontroller_init_slave(void** handler)
{
    ...
    if (mbs_opts == NULL) {
        mbs_opts = malloc(sizeof(mb_slave_options_t));
    }
    MB_ASSERT(mbs_opts != NULL);
    ...
    MB_CHECK((mbs_opts->mbs_event_group != NULL),
    ...
    MB_ASSERT(mbs_opts->mbs_task_handle != NULL); // The task is created but handle is incorrect

    mbcontroller_check_event = mbcontroller_check_event_slave;
    mbcontroller_get_param_info = mbcontroller_get_param_info_slave;
    mbcontroller_set_descriptor = mbcontroller_set_descriptor_slave;
    mbcontroller_setup = mbcontroller_setup_slave;
    mbcontroller_start = mbcontroller_start_slave;
    mbcontroller_destroy = mbcontroller_destroy_slave;

    *handler = (void*)mbs_opts;
