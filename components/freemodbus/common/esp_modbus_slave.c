 ...
 */
inline esp_err_t mbc_slave_setup(void* comm_info)
{
    esp_err_t error = ESP_OK;
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->setup != NULL);
    error = slave_interface_ptr->setup(comm_info);
    MB_SLAVE_CHECK((error == ESP_OK), 
                    ESP_ERR_INVALID_STATE, 
                    "SERIAL slave setup failure error=(0x%x).", 
                    (uint16_t)error);
    return error;
}
...
esp_err_t mbc_slave_start()
{
    esp_err_t error = ESP_OK;
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->start != NULL);
#ifdef CONFIG_MB_CONTROLLER_SLAVE_ID_SUPPORT
    ...
    MB_SLAVE_CHECK((error == ESP_OK), 
                    ESP_ERR_INVALID_STATE, 
                    "SERIAL slave start failure error=(0x%x).", 
                    (uint16_t)error);
    return error;
} 
...
{
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->check_event != NULL);
...
esp_err_t mbc_slave_get_param_info(mb_param_info_t* reg_info, uint32_t timeout)
{
    esp_err_t error = ESP_OK;
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->get_param_info != NULL);
    error = slave_interface_ptr->get_param_info(reg_info, timeout);
    MB_SLAVE_CHECK((error == ESP_OK), 
                    ESP_ERR_INVALID_STATE, 
                    "SERIAL slave get parameter info failure error=(0x%x).", 
                    (uint16_t)error);
    return error;
}
...
esp_err_t mbc_slave_set_descriptor(mb_register_area_descriptor_t descr_data)
{
    esp_err_t error = ESP_OK;
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->set_descriptor != NULL);
    error = slave_interface_ptr->set_descriptor(descr_data);
    MB_SLAVE_CHECK((error == ESP_OK), 
                    ESP_ERR_INVALID_STATE, 
                    "SERIAL slave set descriptor failure error=(0x%x).", 
                    (uint16_t)error);
    return error;
}
                            ...
                            USHORT usNDiscrete)
{
    ...
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->slave_reg_cb_discrete != NULL);
        ...
        USHORT usNCoils, eMBRegisterMode eMode)
{
    eMBErrorCode error = MB_ENOERR;
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->slave_reg_cb_coils != NULL);
        ...
        USHORT usNRegs, eMBRegisterMode eMode)
{
    eMBErrorCode error = MB_ENOERR;
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->slave_reg_cb_holding != NULL);
                                ...
                                USHORT usNRegs)
{
    eMBErrorCode error = MB_ENOERR;
    MB_SLAVE_ASSERT(slave_interface_ptr != NULL);
    MB_SLAVE_ASSERT(slave_interface_ptr->slave_reg_cb_input != NULL);
