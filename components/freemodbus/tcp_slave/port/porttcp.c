...
#define MB_TCP_CONNECTION_TIMEOUT   ( 10000000 ) // disconnect timeout in uS
...
vMBTCPPortServerTask(void *pvParameters)
{
    ...
    while (1) {
        ...
        while(1) {
            ...
            tv.tv_sec = (MB_TCP_CONNECTION_TIMEOUT / 1000000);
            ...
            // then drop connection and restart
            if (xErr != ERR_OK) {
                uint64_t xTimeStamp = xMBTCPGetTimeStamp();
                // If data update is timed out
                if (xTimeStamp >= (xLastResponseTimeStamp + MB_TCP_CONNECTION_TIMEOUT)) {
