...
static esp_err_t mbcontroller_setup_master(void* comm_info)
{
    MB_ASSERT(mbm_opts != NULL);
    const mb_communication_info_t* comm_info_ptr = (mb_communication_info_t*)comm_info;
    // Check communication options
    MB_CHECK(((comm_info_ptr->mode == MB_MODE_RTU) || (comm_info_ptr->mode == MB_MODE_ASCII)),
                ESP_ERR_INVALID_ARG, "mb incorrect mode = (0x%x).",
                (uint32_t)comm_info_ptr->mode);
    MB_CHECK((comm_info_ptr->port <= UART_NUM_2), ESP_ERR_INVALID_ARG,
                "mb wrong port to set = (0x%x).", (uint32_t)comm_info_ptr->port);
    MB_CHECK((comm_info_ptr->parity <= UART_PARITY_EVEN), ESP_ERR_INVALID_ARG,
                "mb wrong parity option = (0x%x).", (uint32_t)comm_info_ptr->parity);
    // Save the communication options
    mbm_opts->mbm_comm = *comm_info_ptr;
    return ESP_OK;
}

// Modbus controller stack start function
static esp_err_t mbcontroller_start_master(void)
{
    eMBErrorCode status = MB_EIO;

    MB_ASSERT(mbm_opts != NULL);
    const mb_communication_info_t* comm_info = &mbm_opts->mbm_comm;
...
static esp_err_t mbcontroller_destroy_master(void)
{
    MB_ASSERT(mbm_opts != NULL);
    ...
    free(mbm_opts); // free the memory allocated for options

    return ESP_OK;
}

// Set Modbus parameter description table
static esp_err_t mbcontroller_set_descriptor_master(const mb_parameter_descriptor_t* descriptor, const uint16_t num_elements)
{
...
static esp_err_t mbcontroller_send_request_master(mb_param_request_t* request, void* data_ptr)
{
    MB_ASSERT(mbm_opts != NULL);
...
static esp_err_t mbcontroller_get_cid_info_master(uint16_t cid, const mb_parameter_descriptor_t** param_buffer)
{
    MB_ASSERT(mbm_opts != NULL);
    MB_CHECK((param_buffer != NULL), ESP_ERR_INVALID_ARG, "mb incorrect data buffer pointer.");
    MB_CHECK((mbm_opts->mbm_param_descriptor_table != NULL), ESP_ERR_INVALID_ARG, "mb incorrect descriptor table or not set.");
    MB_CHECK((cid < mbm_opts->mbm_param_descriptor_size), ESP_ERR_NOT_FOUND, "mb incorrect cid of characteristic.");
                                                ...
                                                mb_parameter_descriptor_t* reg_data)
{
    ...
    MB_ASSERT(mbm_opts->mbm_param_descriptor_table != NULL);
    ...
    for (uint16_t counter = 0; counter < (mbm_opts->mbm_param_descriptor_size); counter++, reg_ptr++)
    {
        ...
        // Todo: we don't need to keep cid in the table if order equal to record number. Update.
...
static esp_err_t mbcontroller_get_parameter_master(uint16_t cid, char* name, uint8_t* value, uint8_t *type)
{
    ...
    if ((error == ESP_OK) && (cid == reg_info.cid)) {
        error = mbcontroller_send_request_master(&request, &param_buffer[0]);
        if (error == ESP_OK) {
...
static esp_err_t mbcontroller_set_parameter_master(uint16_t cid, char* name, uint8_t* value, uint8_t *type)
{
    MB_CHECK((name != NULL), ESP_ERR_INVALID_ARG, "mb incorrect descriptor.");
...
esp_err_t mbcontroller_init_master(void** handler)
{
    ...
    if (mbm_opts == NULL) {
        mbm_opts = malloc(sizeof(mb_master_options_t));
    }
    MB_ASSERT(mbm_opts != NULL);
    ...
    MB_CHECK((mbm_opts->mbm_event_group != NULL), ESP_ERR_NO_MEM, "mb event group error.");
    ...
    MB_ASSERT(mbm_opts->mbm_task_handle != NULL); // The task is created but handle is incorrect

    mbcontroller_destroy = mbcontroller_destroy_master;
    mbcontroller_start = mbcontroller_start_master;
    mbcontroller_set_parameter = mbcontroller_set_parameter_master;
    mbcontroller_get_parameter = mbcontroller_get_parameter_master;
    mbcontroller_get_cid_info = mbcontroller_get_cid_info_master;
    mbcontroller_send_request = mbcontroller_send_request_master;
    mbcontroller_set_descriptor = mbcontroller_set_descriptor_master;
    mbcontroller_setup = mbcontroller_setup_master;
