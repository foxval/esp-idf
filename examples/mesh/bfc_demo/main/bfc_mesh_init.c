...
bool MESH_FUNC_ATTR mesh_router_init()
{
    ...
    MESH_MEMCPY(sta.password, MESH_ROUTER_PASSWD,
    MESH_STRLEN(MESH_ROUTER_PASSWD));
...
bool MESH_FUNC_ATTR mesh_usr_init()
{
     ...
     */
    if (!esp32_mesh_encrypt_init(MESH_AUTH, (uint8_t *) MESH_PASSWD,
    MESH_STRLEN(MESH_PASSWD))) {
     ...
     * mesh_group_id and mesh_ssid_prefix represent mesh network
     */
    if (!esp32_mesh_group_id_init((uint8_t *) MESH_GROUP_ID,
     ...
     */
    if (!esp32_mesh_server_init((ip_addr_t *) MESH_SERVER_IP,
            MESH_SERVER_PORT)) {
