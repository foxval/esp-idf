...
static void MESH_FUNC_ATTR mesh_performance_bench_tx(uint16_t len, uint32_t pkt_count)
{
    uint32_t i;
    double bps, pps;
    uint32_t pkts = 0;
    ...
    if (esp32_mesh_get_hop() == ESP_MESH_HOP_ONE) {
        ...
        MESH_MEMCPY(dst + sizeof(g_mesh_server_info.ip), &g_mesh_server_info.port, sizeof(g_mesh_server_info.port));
    } else {
    ...
    while (1) {
        ...
        for (i = 0; i < pkt_count; i ++) {
            if (esp32_mesh_send(header, header->len, 10000) <= 0)
                break;
